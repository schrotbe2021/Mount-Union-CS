...
public class NorthFirstMazeSolver extends MazeSolver implements Runnable {
    ...
    public void solveMaze() {
        ...
        //  instantiate a new StackOfObjs with an initial capacity that is twice the total number
        ...
        while(visitedCells < totCells && haveNotReachedEndingSquare()) {
            ...
            if(!unvisitedList.isEmpty()) {
				...
				//        NORTH means "row-1")
                if (unvisitedList.get(0).equals("N"))
                    row = row - 1;
                if(unvisitedList.get(0).equals("S"))
                    row = row + 1;
                if(unvisitedList.get(0).equals("E"))
                    col = col + 1;
                if(unvisitedList.get(0).equals("W"))
                    col = col - 1;
